...
def build_speech_system_prompt(game, player, personality: str = "", prior_speeches: list[dict] | None = None, speech_order: list[int] | None = None) -> str:
    """构造白天发言的系统 prompt。
    ...
    speech_order_block = ""
    if speech_order:
        pos = speech_order.index(player.player_id) + 1 if player.player_id in speech_order else 0
        total = len(speech_order)
        already_spoken = [s['player_id'] for s in (prior_speeches or [])]
        upcoming = [pid for pid in speech_order if pid not in already_spoken and pid != player.player_id]
        lines = [
            "发言规则：从最小号玩家开始，按编号从小到大依次发言。",
            "每个玩家每天只能发一次言，已经发完言的玩家不会再发言。",
            f"你是第 {pos} 个发言的（共 {total} 人发言）。",
        ]
        if already_spoken:
            lines.append("已经发完言的玩家（不会再次发言）：" + "、".join(f"{pid}号" for pid in already_spoken))
        if upcoming:
            lines.append("在你之后发言的玩家：" + "、".join(f"{pid}号" for pid in upcoming))
        else:
            lines.append("你是最后一个发言的。")
        lines.append("注意：比你号数小的玩家已经在前面发言了，不会再发言。你可以放心回应他们。")
        speech_order_block = "\n【发言顺序】\n" + "\n".join(lines) + "\n"
...
def build_review_prompt(game, player) -> str:
    """构造游戏结束后的复盘 prompt。AI 玩家反思自己的操作并评价其他玩家。"""
    from ..game.roles import is_wolf
    ...
    for rec in game.full_record:
        ...
        t = rec["type"]
        if t == "death" and rec.get("died"):
            record_lines.append(f"{d}天亮 {('、'.join(str(x) + '号' for x in rec['died']))} 死亡")
        elif t == "execution":
            record_lines.append(f"{d}白天 {rec.get('executed')}号玩家被投票放逐")
        elif t == "vote_round":
            v = rec.get("votes") or []
            vote_summary = "、".join(f"{x['voter']}号→{('弃权' if not x['target'] else str(x['target']) + '号')}" for x in v)
            record_lines.append(f"{d}投票：{vote_summary}")
            if rec.get("thinking"):
                for pid, th in rec["thinking"].items():
                    record_lines.append(f"  {pid}号思考：{th}")
        elif t == "speech_round":
            for sp in rec.get("speeches", []):
                text = sp.get("text", "").replace("\n", " ")
                record_lines.append(f"{d}白天 {sp['player_id']}号发言：{text}")
        elif t == "night_thinking":
            for pid, th in rec.get("thinking", {}).items():
                record_lines.append(f"{d}夜晚 {pid}号决策思考：{th}")
        elif t == "divine":
            record_lines.append(f"{d}夜晚 预言家({rec.get('seer')}号)查验了{rec.get('target')}号 → {rec.get('result')}")

    record_block = "\n".join(record_lines) if record_lines else "无记录"

    # 所有玩家的角色
    roles_block = "、".join(f"{pid}号={p.role}({'存活' if p.alive else '死亡'})" for pid, p in game.players.items())

    # 自己的关键决策
    my_decisions = []
    for rec in game.full_record:
        if rec.get("type") == "vote_round" and rec.get("thinking"):
            th = rec["thinking"].get(player.player_id)
            if th:
                my_decisions.append(f"第{rec['day']}天投票思考：{th}")
        if rec.get("type") == "night_thinking" and rec.get("thinking"):
            th = rec["thinking"].get(player.player_id)
            if th:
                my_decisions.append(f"第{rec['day']}天夜晚决策：{th}")
    my_decisions_block = "\n".join(my_decisions) if my_decisions else "无记录"

    return f"""你是 {player.player_id} 号玩家，角色是 {player.role}（{camp}阵营）。
游戏已经结束，{game.winner}获胜。
...
【复盘要求】
请以第一人称写一段复盘（300-600字），包含以下内容：
1. 对自己操作的反思：你在关键决策（投票/夜间行动/发言）中做对了什么、做错了什么？如果重来你会怎么改？
2. 对其他玩家的评价：谁的操作给你留下深刻印象？谁打得好、谁打得差？为什么？
3. 整体感受：这局游戏你觉得最关键的转折点是什么？

请自然地写，像和朋友聊天一样，不要用列表格式。"""
